...
def economic_score_model(prev_econ_score, current_revenue, environment_score, social_score, alpha=0.3, max_econ_score=100):
    base_part = (current_revenue / total_revenue) * 20000  # 增加缩放因子
    synergy_factor = 0.03
    bonus = synergy_factor * (environment_score + social_score)
    new_score = alpha * prev_econ_score + (1 - alpha) * (base_part + bonus)
    new_score = min(new_score, max_econ_score)
    new_score = max(-10, new_score)
    return new_score
...
# -------------------- 优化函数（修改为互相影响版本） --------------------
def optimize_sustainability_with_history(budget, env_threshold=19, soc_threshold=26,
    ...
    if result.success:
        ...
        best_allocation = {
            ...
            'Environment Score': round(tmp_env_score, 2),
            'Social Score': round(tmp_soc_score, 2),
...
# -------------------- 可持续性评分优化后的饼图绘制函数 --------------------
def plot_optimal_scores(allocation):
    labels = ['Economic Score', 'Environment Score', 'Social Score']
    sizes = [allocation['Economic Score'],
             allocation['Environment Score'],
             allocation['Social Score']]
    colors = ['#4D4D4D', '#5DA5DA', '#FAA43A']
    explode = (0.05, 0.05, 0.05)  # 略微分离各部分

    # 归一化尺寸，使其总和为100
    total = sum(sizes)
    sizes_normalized = [size / total * 100 for size in sizes]

    plt.figure(figsize=(8, 8))
    plt.pie(sizes_normalized, explode=explode, labels=labels, colors=colors, autopct='%1.1f%%',
            shadow=True, startangle=140)
    # plt.title('Optimized Sustainability Scores')
    plt.axis('equal')
    plt.show()
...
def sensitivity_analysis_combined():
    ...
    results = []
    for env, soc, stab in product(env_factors, social_factors, stability_factors):
        # 计算对应的 Economic Score 基于环境和社会评分
        # 假设 revenue 与 env + soc 成正比，这里用简单的线性关系作为示例
        # 可以根据实际情况调整
        current_revenue = (env + soc) * 1000  # 示例计算
        econ = economic_score_model(
            economic_score_initial,
            current_revenue,
            env,
            soc,
...
def plot_optimal_scores(allocation):
    ...
    # 确保各评分不超过100
    sizes_scaled = [size if size <= 100 else 100 for size in sizes]
    ...
    # 设置标签的字体大小
    for text in texts:
        text.set_fontsize(20)  # 调整为所需的字体大小，例如14

    # 设置百分比文本的字体大小
    for autotext in autotexts:
        autotext.set_fontsize(20)  # 调整为所需的字体大小，例如12

    # （可选）如果需要标题，也可以设置标题的字体大小
    # plt.title('Optimized Sustainability Scores', fontsize=16)
...
if optimal_allocation is not None:
    plot_optimal_scores(optimal_allocation)
else:
...
print("2) 经济评分放大对环境与社会的依赖，可见 synergy_factor=0.03；")
